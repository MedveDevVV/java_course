...
public class ShowOrdersAction implements IAction {
    private static final Logger logger = LoggerFactory.getLogger(ShowOrdersAction.class);
    ...
    @Override
    public void execute() {
        logger.info("Начало выполнения: просмотр заказов с фильтрацией");
        ...
        try {
            ...
            switch (choice) {
                case 1:
                    ...
                    logger.info("Фильтр: по статусу {}", OrderStatus.values()[statusChoice]);
                    break;

                case 2:
                    List<CarServiceMaster> masters = admin.getCarServiceMasters();
                    if (masters.isEmpty()) {
                        logger.warn("Попытка фильтрации по мастеру: нет доступных мастеров");
                    ...
                    int choiceMaster = InputUtils.readNumberInRange(
                            scanner, "Выберите номер мастера: ", 1, masters.size());
                    builder.carServiceMaster(masters.get(choiceMaster - 1));
                    logger.info("Фильтр: по мастеру {}", masters.get(choiceMaster - 1).getFullName());
                    break;
                ...
                case 3:
                    ...
                    logger.info("Фильтр: за период с {} по {}", startDate, endDate);
                    break;
            }
            ...
            logger.info("Сортировка: {}", SortRepairOrders.values()[sortChoice]);
            ...
            logger.info("Команда 'просмотр заказов с фильтрацией' успешно выполнена. Найдено заказов: {}",
                    orders.size());
        } catch (Exception e) {
            logger.error("Ошибка выполнения команды 'просмотр заказов с фильтрацией'", e);
