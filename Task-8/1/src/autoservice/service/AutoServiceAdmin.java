...
public class AutoServiceAdmin {
    ...
    public void loadState(ApplicationState state) {
        if (!state.getMasters().isEmpty())
            masterRepository.setAllMasters(state.getMasters());

        if (!state.getPlaces().isEmpty())
            workshopPlaceRepository.setAllPlaces(state.getPlaces());

        if (!state.getRepairOrders().isEmpty())
            repairOrderRepository.setAllOrders(state.getRepairOrders());
    }
