...
public class ListMastersAction implements IAction {
    private static final Logger logger = LoggerFactory.getLogger(ListMastersAction.class);
    ...
    @Override
    public void execute() {
        logger.info("Начало выполнения: список мастеров");
        ...
        try {
            ...
            logger.info("Вывод списка мастеров. Вариант: {}, количество: {}", choice, masters.size());
            ...
            logger.info("Команда 'список мастеров' успешно выполнена. Выведено мастеров: {}", masters.size());

        } catch (Exception e) {
            logger.error("Ошибка выполнения команды 'список мастеров'", e);
